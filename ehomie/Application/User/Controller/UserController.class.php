...
class UserController extends HomeController {
        ...
        if (IS_POST) {
            ...
            if ($data) {
                ...
                if ($id) {
                    ...
                    $this->success('注册成功', U('Rent/Mark/my'));
                } else {
    ...
    public function resetPassword() {
        if (IS_POST) {
            ...
            if ($user_info) {
                ...
                $msg_data['content'] = '尊敬的用户：<br>'
                                      .'您好。恭喜您成功重置您在'.C('WEB_SITE_TITLE').'的帐号密码<br>'
                                      .'您的帐号信息如下（请妥善保管）：<br>'
                                      .'UID：'.$user_info['id'].'<br>'
    ...
    public function send_mail_verify(){
        ...
        if(I('post.title') == '用户注册') {
            $this->exist_check(I('post.email'));
        }
        ...
        $mail_data['content'] = '尊敬的用户:<br />您好，请使用下面的验证码验证您的邮箱。<br/>您的验证码是: <span style="color:red;font-weight:bold;">'.$reg_verify.'</span> 如果并非本人注册，请忽略本邮件。';
     ...
     * @author jry <598821125@qq.com>
     */
    public function exist_check($email)
    {
        $map['email'] = $email;
        $map['status'] = array('gt',0);
        $result = D('User/User')->where($map)->find();
        if($result) {
            $this->error('该邮箱已注册！');
        }
    }
